  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!existingActor) {
        return res
          .status(400)
          .json({ errors: [{ msg: "Actor does not exist" }] });
      }
      ...
      if (existingOffre) {
        return res
          .status(400)
          .json({ error: "You can only add one offre per day." });
      }
...
router.get("/download", (req, res) => {
  ...
  res.download(filePath, `Offre.pdf`, (err) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Error: Unable to download the PDF file" });
    }
  });
});
