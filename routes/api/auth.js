...
const {
  ...
  processPDF,
  convertToPDF,
} = require("../../Functions/PdfFunctions");
...
router.post("/validate", async (req, res) => {
  ...
  if (!number || !newPassword) {
    return res
      .status(400)
      .json({ errors: [{ msg: "Number and new password are required" }] });
  }
  ...
  try {
    ...
    if (decoded.randomNumber !== number) {
      return res
        .status(400)
        .json({ success: false, message: "Validation failed. Invalid code." });
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    let { nom, prenom, telephone, email, willaya, category, password } =
      req.body;
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const actors = await Actor1.find(filter).skip(skip).limit(limit);
...
router.put("/updatesos/:id", async (req, res) => {
  console.log("Request body: ", req.body); // This should show the body content
  console.log("Request params: ", req.params); // Check params if needed

  const { status, offre } = req.body; // Ensure 'offre' is being passed
  console.log(req.body); // Debugging line

  try {
    let actor = await Actor1.findById(req.params.id); // Get actor by ID
...
router.put("/update-image/:id", upload.single("file"), async (req, res) => {
  const { id } = req.params;
